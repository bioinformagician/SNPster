...
import tempfile
import shutil
from config import REFERENCE_VCF_DIR, POPULATION_PANEL_FILE, DEFAULT_CHROMOSOME, K_POPULATIONS, ANCESTRY_METHOD, REFERENCE_PANEL
...
class AncestryEnvironmentHandler:
    def __init__(
        ...
        output_dir: str = ".",
        reference_vcf_dir: str = REFERENCE_VCF_DIR,  # Your existing 1000G VCFs
        population_panel_file: str = POPULATION_PANEL_FILE,  # Population assignments
        use_chromosome: str = str(DEFAULT_CHROMOSOME),  # Use chr22 for speed (smallest autosome)
...
class AncestryInference:
    ...
    def _get_reference_vcf_for_chr(self, chrom: str) -> str:
        ...
        # Pattern: 1kGP_high_coverage_Illumina.chr22.filtered.SNV_INDEL_SV_phased_panel.numericCHR.SNV_INDEL_biallelic.vcf.gz
        pattern = f"1kGP_high_coverage_Illumina.chr{chrom}.filtered.SNV_INDEL_SV_phased_panel.numericCHR.SNV_INDEL_biallelic.vcf.gz"
    ...
    def _merge_sample_with_reference(self, sample_vcf: str, ref_vcf: str, output_prefix: str) -> str:
        """Merge sample VCF with 1000G reference VCF at common positions."""
        print(f"Merging sample with 1000 Genomes reference...")
        
        merged_vcf = f"{output_prefix}.merged.vcf.gz"
        
        # Merge at intersecting positions only
        subprocess.run([
            "bcftools", "merge",
    ...
    def _create_population_file(self, merged_bed_prefix: str, panel_file: str) -> str:
        """Create .pop file for ADMIXTURE supervised mode."""
        ...
        for _, row in fam_df.iterrows():
            ...
            if sample_id in panel_df['sample'].values:
                ...
                pop_assignments.append(super_pop_map.get(super_pop, '-'))
            else:
    ...
    def _parse_admixture_results(self, q_file: str, fam_file: str, panel_file: str) -> pd.DataFrame:
        """Parse ADMIXTURE results and extract user sample ancestry only."""
        ...
        # Column names for K=5
        ancestry_cols = ['EUR', 'AFR', 'EAS', 'SAS', 'AMR'][:q_df.shape[1]]
        q_df.columns = ancestry_cols
        
        # Combine
        result_df = pd.concat([fam_df, q_df], axis=1)
        
        # Determine primary ancestry using the already-joined ancestry columns
    ...
    def run_ancestry_inference(self) -> pd.DataFrame:
        """Main workflow: infer ancestry using existing 1000G VCF references."""
        
        chrom = self.env.use_chromosome
        
        # Step 1: Get reference VCF for chosen chromosome
        ref_vcf = self._get_reference_vcf_for_chr(chrom)
        print(f"Using reference: {ref_vcf}")
        
        # Step 2: Extract same chromosome from sample VCF
        ...
        # Step 3: Merge sample with reference at common positions
        ...
        self._convert_vcf_to_plink(merged_vcf, plink_prefix)
        
        # Step 5: Create population file for supervised ADMIXTURE
        ...
        # Step 7: Parse results (user samples only)
