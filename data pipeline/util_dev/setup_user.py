...
def create_prsc_jobs_for_all_reports(db_handler: DbHandler, imputation_id: int) -> list[tuple[int, str]]:
	"""
	Create one queued PRSC job per report type and subscribe each job to all pgs_ids
	in that report.
	...
	"""
	report_rows = db_handler.execute_query(report_query)
	if not report_rows:
		raise ValueError("No report types found in snpster_users.pgs_reports_shop.")

	created_jobs: list[tuple[int, str]] = []

	for (report_name,) in report_rows:
		insert_prsc_query = """
		...
		insert_params_query = """
			...
			ON CONFLICT DO NOTHING;
		"""
		db_handler.execute_query(insert_params_query, (prsc_id, report_name))
