...
if __name__ == "__main__":
    ...
    while True:
        ...
        imputation_ids = [int(imputation_id) for imputation_id in results["imputation_id"].tolist()]
        ...
        db_handler.execute_query(mark_running_query)
        ...
        try:
            ...
            mark_completed_query = f"""UPDATE snpster_users.imputation_jobs
                                       ...
                                       WHERE imputation_id IN ({imputation_ids_sql});"""
            db_handler.execute_query(mark_completed_query)
        except subprocess.CalledProcessError:
            mark_failed_query = f"""UPDATE snpster_users.imputation_jobs
                                    SET imputation_status = 'failed',
                                        completed_at = CURRENT_TIMESTAMP
                                    WHERE imputation_id IN ({imputation_ids_sql});"""
            db_handler.execute_query(mark_failed_query)
            print("Nextflow run failed. Marked jobs as failed and retrying polling loop.")
        finally:
            db_handler.close()